(function () {
  ...
  */
  ...
  fnSetQuiz = function () {
    ...
    var i,
      ...
      fnSetElements = function (a, sName) {
        JSLITE.dom.gets("#" + sName + " a").remove(); //first remove everything but the title in the <p>, i.e., "Type" and "Inversion";
  ...
  var fnReset = function () {
    var arr = JSLITE.makeArray($("notes").childNodes).concat(JSLITE.makeArray(JSLITE.dom.gets("#dropZoneContainer .JSLITE_draggable", true))).filter(function (v) { //get all child nodes within the drop zone that have a "sortOrder" property;
      return (typeof v.sortOrder === "number"); //should there be a better check?;
    }),
    oFrag = document.createDocumentFragment(),
    oDropZone = JSLITE.dom.get("notes");
  ...
  JSLITE.ready(function () {
    ...
    */
    JSLITE.ux.DropZoneManager.add(JSLITE.dom.gets("#notes"), {
      sort: true
    });

    JSLITE.ux.DropZoneManager.add(JSLITE.dom.gets(".dropZone"), {
      snapToZone: true,
      subscribe: {
        ...
        afternodedrop: function (e) {
          var aDragged = JSLITE.dom.gets("#dropZoneContainer .JSLITE_draggable", true),
            i,
            len,
            arr = [];
    ...
    JSLITE.dom.fly("chordQuiz").on("click", function (e) { //note we're only binding one event listener for the entire page (because of this make sure each <span> entirely covers each <a>);
      ...
      if (oTarget.nodeName === "SPAN" && oTarget.className !== "blank") {
        JSLITE.dom.gets("span", $(JSLITE.trim(oTarget.className))).removeClass("selected"); //the classname needs to be trimmed b/c if it removeClass() was previously called on this then there will be an extra space in the classname, i.e., "notes " (is this a bug?);
        JSLITE.dom.fly(oTarget).addClass("selected");
        if (JSLITE.dom.get("#notes span.selected", true) && JSLITE.dom.get("#chords span.selected", true) && JSLITE.dom.get("#inversions span.selected", true)) { //user selected one of each so see if he selected correctly;
          sNote = JSLITE.dom.get("#notes .selected").dom.note;
          sChord = JSLITE.dom.get("#chords .selected").value();
          sInversion = JSLITE.dom.get("#inversions .selected").value().replace(/\s/, ""); //remove the space that was put in when the dom element was created, i.e., "Second Inversion";
          ...
          JSLITE.dom.gets("span").removeClass("selected");
          if ($("beginner").checked) { //if beginner skill level is selected, make sure the "Root Position" element is given the selected class (to understand why see the logic w/in the handler bound to the "chordQuiz" element);
            JSLITE.dom.get("#inversions span").addClass("selected");
          }
        }
        e.preventDefault();
      }
    ...
    JSLITE.dom.get(".skipChord").on("click", skip);
  
    JSLITE.dom.gets("input[type=radio]").on("click", function (e) {
      var oForm = e.target.form,
        sValue = e.target.value;
  
      if (oForm.id === "mainMenu") {
        var oChordQuiz = JSLITE.dom.get("chordQuiz"),
          oKeyQuiz = JSLITE.dom.get("keySignaturesQuiz"),
          fnToggleElements = function (sDiv) {
            ...
            oChordQuiz.show();
            JSLITE.dom.gets("#notes span").removeClass("selected"); //no matter which "view" was selected remove any previously selected notes;
            JSLITE.dom.get("#chordQuiz h3 + h3", true).style.display = $("dropZoneContainer").style.display = a[0];
            JSLITE.dom.get("#chordQuiz h3", true).style.display = $("chordMenu").style.display = $("chords").style.display = $("inversions").style.display = a[1];
            fnReset(); //finally, reset the notes div (in case any were dragged in the chord builder);
          };
   
        if (sValue === "chordQuiz") {
          fnToggleElements("chordQuiz");
          JSLITE.dom.get("h3 + div + p").hide(); //hide the first paragraph;
          JSLITE.dom.gets("p + div + p").show(); //show every other paragraph;
          JSLITE.dom.gets("#notes a.notes").removeClass("JSLITE_draggable");
          oKeyQuiz.hide();
          init();
          ...
          JSLITE.dom.get("h3 + div + p").show(); //show the first paragraph;
          JSLITE.dom.gets("p + div + p").hide(); //hide every other paragraph;
          JSLITE.dom.gets("#notes a.notes").addClass("JSLITE_draggable");
          oKeyQuiz.hide();
          init(sValue);
        ...
        JSLITE.dom.get("#inversions span").removeClass("selected");
