function Scrubber(obj) {
  ...
  //public properties;
  if (obj.constructor === Object && !formSubmit) { //probably the event handler was 'onchange';
    this.theForm = obj.elem;
	this.action = obj.evt;
	this.formObj = obj.formObj;
	
  } else if (obj.constructor === Object && formSubmit) {
	this.theForm = obj.formObj;

  } else {
    this.theForm = obj;
	allErrors = []; //destroy the temp string holder;
	formSubmit = true;
  }
...
Scrubber.prototype.validate = function() {
  ...
  this.increment = function() { //keep track of the total number of errors (useful for events such as 'onblur');
  ...
  if (this.errors.length == 0 && this.theForm === Object) { //only flag 'allGood' if there are no errors and 'this.theForm' contains the Form object;
    allGood = true;
  } else if (this.errors.length > 0 && this.action) {
    if (allErrors.join().indexOf(this.errors.join()) == -1) {
  ...
  if (this.errors.length > 0) {
   if (args) {
    ...
    if (args.div) { //construct an error list to be inserted into the DOM; 
	  ...
	  var errorBlock = allErrors.length > 0 ? allErrors : this.errors;
	  //alert(errorBlock);
	  for (var i = 0; i < errorBlock.length; i++) {
		  //alert(errorBlock[i][0]);
		  //alert(errorBlock[i][1]);
		//capitalize the first letter of the field name(s);
	    html += "\t\t<li><strong>" + this.capFirstLetter(errorBlock[i][0]) + "</strong>" + errorBlock[i][1] + "</li>\n";
	  }
      html += "\t</ul>\n";
	}
